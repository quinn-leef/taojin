...
public interface PrefFeign {
    ...
    Result<PageInfo> findPage(@RequestBody(required = false) Pref pref, @PathVariable("page") int page, @PathVariable("size") int size);
     ...
     */
    @PutMapping(value="/{id}")
    Result update(@RequestBody Pref pref, @PathVariable("id") Integer id);
     ...
     */
    @GetMapping("/{id}")
    Result<Pref> findById(@PathVariable("id") Integer id);
