...
public interface TemplateFeign {
    ...
    Result<PageInfo> findPage(@RequestBody(required = false) Template template, @PathVariable("page") int page, @PathVariable("size") int size);
     ...
     */
    @PutMapping(value="/{id}")
    Result update(@RequestBody Template template, @PathVariable("id") Integer id);
     ...
     */
    @GetMapping("/{id}")
    Result<Template> findById(@PathVariable("id") Integer id);
