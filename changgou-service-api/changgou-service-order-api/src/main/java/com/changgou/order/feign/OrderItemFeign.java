...
public interface OrderItemFeign {
    ...
    Result<PageInfo> findPage(@RequestBody(required = false) OrderItem orderItem, @PathVariable("page") int page, @PathVariable("size")  int size);
     ...
     */
    @PutMapping(value="/{id}")
    Result update(@RequestBody OrderItem orderItem,@PathVariable("id") String id);
     ...
     */
    @GetMapping("/{id}")
    Result<OrderItem> findById(@PathVariable("id") String id);
