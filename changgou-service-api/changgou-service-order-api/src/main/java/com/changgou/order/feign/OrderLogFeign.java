...
public interface OrderLogFeign {
    ...
    Result<PageInfo> findPage(@RequestBody(required = false) OrderLog orderLog, @PathVariable("page") int page, @PathVariable("size")  int size);
     ...
     */
    @PutMapping(value="/{id}")
    Result update(@RequestBody OrderLog orderLog,@PathVariable("id") String id);
     ...
     */
    @GetMapping("/{id}")
    Result<OrderLog> findById(@PathVariable("id") String id);
