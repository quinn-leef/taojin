...
public interface OrderConfigFeign {
    ...
    Result<PageInfo> findPage(@RequestBody(required = false) OrderConfig orderConfig, @PathVariable("page") int page, @PathVariable("size")  int size);
     ...
     */
    @PutMapping(value="/{id}")
    Result update(@RequestBody OrderConfig orderConfig,@PathVariable("id") Integer id);
     ...
     */
    @GetMapping("/{id}")
    Result<OrderConfig> findById(@PathVariable("id") Integer id);
