...
public interface PreferentialFeign {
    ...
    Result<PageInfo> findPage(@RequestBody(required = false) Preferential preferential, @PathVariable("page") int page, @PathVariable("size")  int size);
     ...
     */
    @PutMapping(value="/{id}")
    Result update(@RequestBody Preferential preferential,@PathVariable("id") Integer id);
     ...
     */
    @GetMapping("/{id}")
    Result<Preferential> findById(@PathVariable("id") Integer id);
